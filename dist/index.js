...
import { getPortfolio, getTokenPrice, formatBalance, formatUSD, B20_TOKENS } from './base.js';
import { getSwapQuote, parseAmount, formatAmount } from './swap.js';
import { sendAgentMessage, getTradingMemory, setTradingMemory } from './letta.js';
import { analyzePortfolio, formatAnalytics, getPriceChanges } from './analytics.js';
import { getTransactionHistory, formatTransactionHistory } from './history.js';
import { handleStopLoss, handleRebalance, handleSentiment } from './automation.js';
...
function formatPortfolioMessage(portfolio) {
...
function formatPriceMessage(symbol, priceData) {
    if (!priceData) {
...
async function handlePortfolio(space, userId) {
...
async function handlePrice(space, symbol) {
...
// Handle buy command
async function handleBuy(space, userId, args) {
    const session = getSession(userId);
    const walletAddress = await initializeWallet(userId);
    if (!walletAddress) {
        await space.send('🔐 Please connect your wallet first. Send "/connect" to get started.');
    ...
    // Validate tokens
    const fromTokenAddress = toToken === 'USDC' ? '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913' : B20_TOKENS[fromToken];
    const toTokenAddress = B20_TOKENS[toToken];
    if (!toTokenAddress) {
...
// Handle sell command
async function handleSell(space, userId, args) {
    const session = getSession(userId);
    const walletAddress = await initializeWallet(userId);
    if (!walletAddress) {
        await space.send('🔐 Please connect your wallet first. Send "/connect" to get started.');
    ...
    const fromTokenAddress = B20_TOKENS[fromToken];
    const toTokenAddress = toToken === 'USDC' ? '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913' : B20_TOKENS[toToken];
    if (!fromTokenAddress) {
    ...
    // Check balance first
    const portfolio = await getPortfolio(walletAddress);
    const holding = portfolio.find((h) => h.symbol === fromToken);
...
// Handle confirm
async function handleConfirm(space, userId) {
    const session = getSession(userId);
    ...
    if (DEMO_MODE === 'true') {
        ...
        await space.send(`✅ **Trade Executed (Demo)**\n\n` +
...
// Handle cancel
async function handleCancel(space, userId) {
    const session = getSession(userId);
...
// Handle watchlist
async function handleWatchlist(space, userId, args) {
    const memory = await getTradingMemory(userId);
    if (args[0] === 'add' && args[1]) {
        const symbol = args[1].toUpperCase();
        if (B20_TOKENS[symbol]) {
        ...
        const symbol = args[1].toUpperCase();
        memory.watchlist = memory.watchlist.filter((s) => s !== symbol);
        await setTradingMemory(userId, { watchlist: memory.watchlist });
        ...
        const prices = await Promise.all(memory.watchlist.map(async (symbol) => {
...
async function handleDCA(space, userId, args) {
    const memory = await getTradingMemory(userId);
    if (args[0] === 'create' && args.length >= 4) {
        ...
        await space.send(`✅ **DCA Strategy Created**\n\n` +
            `💰 Amount: ${amount} USDC\n` +
            `📈 Token: ${token}\n` +
            `🔄 Frequency: ${frequency}\n` +
            `🟢 Status: Active`);
        ...
        const dcaStrategies = memory.activeStrategies.filter((s) => s.type === 'dca');
...
// Handle alerts
async function handleAlert(space, userId, args) {
    if (args[0] === 'create' && args.length >= 3) {
        ...
        memory.activeStrategies.push(alert);
        await setTradingMemory(userId, { activeStrategies: memory.activeStrategies });
        await space.send(`🔔 **Price Alert Created**\n\n` +
            `📈 Token: ${token}\n` +
            `📊 Trigger: Price goes ${direction} $${price.toFixed(2)}\n` +
            `🟢 Status: Active`);
    }
    else if (args[0] === 'list') {
        const alerts = (await getTradingMemory(userId)).activeStrategies.filter((s) => s.type === 'price_alert');
        if (alerts.length === 0) {
        ...
        await space.send(`🔔 **Alert Commands**\n\n` +
...
// Handle analytics
async function handleAnalytics(space, userId, args) {
...
// Handle history
async function handleHistory(space, userId, args) {
...
// Handle help
async function handleHelp(space) {
    await space.send(`🤖 **Moni - Your iMessage Trading Agent**\n\n` +
        `**Portfolio & Prices**\n` +
...
// Handle connect
async function handleConnect(space, userId) {
    const session = getSession(userId);
    if (session.authenticated && session.walletAddress) {
        await space.send(`✅ **Wallet Connected**\n\n` +
    ...
    if (DEMO_MODE === 'true') {
        ...
        const demoAddress = '0x742d35Cc6634C0532925a3b8D4C0532925a3b8D4';
        session.walletAddress = demoAddress;
        session.authenticated = true;
        await space.send(`✅ **Demo Wallet Connected**\n\n` +
        ...
        await space.send(`🔐 **Connect Your Wallet**\n\n` +
...
async function handleNaturalLanguage(space, userId, message) {
...
async function handleMessage(space, userId, text) {
    const session = getSession(userId);
