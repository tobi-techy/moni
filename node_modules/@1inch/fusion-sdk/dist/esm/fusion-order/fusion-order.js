...
function encodeChainedPostInteraction(chainedPostInteraction) {
    ...
    try {
        for(var _iterator = interactions[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true){
            ...
            assert(isHexBytes(chunk), "invalid chained post-interaction chunk ".concat(chunk, ": must be 0x-prefixed hex bytes of even length"));
        }
    } catch (err) {
    ...
    var encoded = "0x".concat(interactions.map(trim0x).join(''));
