...
export var FusionExtension = /*#__PURE__*/ function() {
     ...
     */ function decodeInteraction(bytes) {
                assert(isHexBytes(bytes) && trim0x(bytes).length >= 40, "invalid interaction ".concat(bytes, ": must be valid hex bytes starting with a 20 bytes target address"));
                var target = Address.fromFirstBytes(bytes);
                var data = add0x(bytes.slice(42));
                if (data === '0x') {
                    return new TargetOnlyInteraction(target);
                }
