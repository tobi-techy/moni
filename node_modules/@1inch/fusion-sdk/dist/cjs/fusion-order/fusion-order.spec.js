...
describe('Fusion Order', function() {
    ...
    it.each([
        {
            name: 'target + data',
            chainedPostInteraction: '0x593a321a1b5ff516eb6eee2c752a8ee7097d5119abcdef'
        },
        {
            name: 'target only',
            chainedPostInteraction: '0x593a321a1b5ff516eb6eee2c752a8ee7097d5119'
        }
    ])('should decode fusion order from order + extension with chained post interaction ($name)', function(param) {
        var chainedPostInteraction = param.chainedPostInteraction;
        var extensionContract = new _limitordersdk.Address('0x8273f37417da37c4a6c3995e82cf442f87a25d9c');
        var order = _fusionorder.FusionOrder.new(extensionContract, {
            makerAsset: new _limitordersdk.Address('0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'),
            takerAsset: new _limitordersdk.Address('0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48'),
        ...
        var decoded = _fusionorder.FusionOrder.fromDataAndExtension(order.build(), order.extension);
...
describe('FusionOrder Native', function() {
    it('should correct detect that order is from native asset', function() {
        ...
        var chainId = _constants1.NetworkEnum.ETHEREUM;
...
describe('FusionOrder preInteraction', function() {
    ...
    it.each([
        ...
        [
            _constants1.NetworkEnum.MONAD,
            _constants1.ONE_INCH_LIMIT_ORDER_V4
        ],
        [
            _constants1.NetworkEnum.CRONOS,
            _constants1.ONE_INCH_LIMIT_ORDER_V4
        ],
        [
            _constants1.NetworkEnum.HYPEREVM,
            '0x5281602adc446a94eb48d055f514a6d8d5bee176'
        ]
    ])('should use correct verifying contract for chain %d', function(chainId, expectedVerifyingContract) {
        var extensionContract = new _limitordersdk.Address('0x8273f37417da37c4a6c3995e82cf442f87a25d9c');
        var order = _fusionorder.FusionOrder.new(extensionContract, {
        ...
        var typedData = order.getTypedData(chainId);
        expect(typedData.domain.chainId).toEqual(chainId);
        expect(typedData.domain.verifyingContract).toEqual(expectedVerifyingContract);
    });
});
